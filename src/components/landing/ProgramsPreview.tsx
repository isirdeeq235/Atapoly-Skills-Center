...
const ProgramCard = memo(({ program, index, isVisible }: { 
  program: { id: string; title: string; description: string | null; image_url: string | null; duration: string | null; enrolled_count: number };
  index: number;
  isVisible: boolean;
}) => (
  <Link 
  ...
  >
    <div className="relative bg-card rounded-3xl border border-border overflow-hidden hover:border-accent/40 transition-all duration-300 hover:shadow-2xl hover:shadow-accent/10 hover:-translate-y-2">
      {/* Image */}
      ...
      <div ref={sectionRef} className="container mx-auto px-4 relative">
        ...
        <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6 mb-12 lg:mb-16">
          <div className="max-w-2xl">
            ...
            <h2 
              className={`text-4xl md:text-5xl lg:text-6xl font-bold text-foreground leading-tight transition-all duration-300 delay-50 ${
                isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'
              }`}
            >
